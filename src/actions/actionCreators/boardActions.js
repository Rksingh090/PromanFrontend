...
const BASE_URL = `${ACTIONS.api}/boards/`
...
export const addBoardMember = (boardId, params, token) => (dispatch) => {
  axios.
  patch(BASE_URL+`addmember/${boardId}`, params, {
    headers: {
      'x-auth-token': token
    }
  })
  .then((res) => {
    dispatach({type: ACTIONS.ADD_BOARD_MEMBER, payload: res.data})
  }).catch((err) => {
    dispatch({ type: ACTIONS.ERROR_BOARD, payload: { error: err.message } })
  })
}
